...
# Backups + logs
BACKUP_DIR="${BACKUP_DIR:-$HOME/mac_config_backup_$(date +%s)}"
LOG_DIR="${LOG_DIR:-$BACKUP_DIR/logs}"
mkdir -p "$BACKUP_DIR" "$LOG_DIR" >/dev/null 2>&1 || true

# Step toggles (wizard/flags)
WANT_XCODE=true
WANT_HOSTNAME=true
WANT_HOMEBREW=true
WANT_BREWS=true
...
ensure_sudo_if_needed() {
  # Only ask if a sudo-requiring step is enabled
  local needs=false
...
parse_args() {
  ...
  # If UI step disabled, Dock/Finder are irrelevant
